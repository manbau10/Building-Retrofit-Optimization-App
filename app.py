...
def home_page():
    st.title("🏗️ Building Retrofit Optimization App")
    
    st.markdown("""
    ## Welcome to the Building Retrofit Optimization App
    
    This tool provides different approaches to optimize building retrofit strategies:
    
    ### Available Analysis Methods:
    
    1. **Life Cycle Analysis (LCA) Optimization**
       * Considers the full life cycle impact of retrofit measures
       * Optimizes based on life cycle costs and benefits
       * Includes embodied carbon in total environmental impact
    
    2. **Annual-based Optimization**
       * Focuses on yearly performance metrics
       * Analyzes annual energy savings and costs
       * Considers annual carbon emission reductions
    
    ### How to Use:
    1. Select your desired analysis method from the sidebar
    2. Upload your data or use provided sample datasets
    3. Configure parameters and run optimization
    4. Analyze results and download reports
    
    ### Detailed User Mannual:
...
def optimization_page(optimization_type):
    ...
    params = {
        'E_BASE': st.sidebar.number_input(
            "Energy Baseline (kWh/m²)" if optimization_type == "Annual" else "Life Cycle Energy Baseline (kWh/m²)", 
            value=111.4
        ),
        'C_BASE': st.sidebar.number_input(
            "Carbon Baseline (kgCO₂)" if optimization_type == "Annual" else "Life Cycle Carbon Baseline (kgCO₂)", 
            value=1384.5
        ),
